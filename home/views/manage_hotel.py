...
def Cart(request):
    ...
    if request.method == 'POST' and 'room_excel' in request.FILES:
...
def manage_hotel(request):
    ...
    context = {'user_not_login': user_not_login, 'hotels_page': hotels_page, 'allowed': allowed, 'count': count, 'profile': user_profile, 'categories': categories, 'hotel_type': hotel_type, 'room_type': room_type, 'query_string': query_string, 'error': error}
